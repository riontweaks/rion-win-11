...
namespace RadeonSoftwareSlimmer.Views
{
    ...
    {
        private PostInstallViewModel _viewModel;
        ...
        public PostInstallView()
        {
            ...
            Loaded += (s, e) =>
            {
                _viewModel = DataContext as PostInstallViewModel;
                if (_viewModel == null)
                {
                    _viewModel = new PostInstallViewModel(new FileSystem(), new WindowsRegistry());
        ...
        private async void UserControl_Initialized(object sender, System.EventArgs e)
        {
            if (_viewModel == null)
                _viewModel = DataContext as PostInstallViewModel
                    ?? new PostInstallViewModel(new FileSystem(), new WindowsRegistry());
