...
namespace RadeonSoftwareSlimmer.Models.PostInstall
{
    public class ServiceModel : INotifyPropertyChanged
    {
        ...
        private const string SERVICE_ORIGINAL_START_VALUE_NAME = "RionOriginalStart";
        ...
        private void LoadOriginalStartMode()
        {
            try
            {
                using (IRegistryKey serviceKey = _registry.LocalMachine.OpenSubKey(SERVICES_REG_KEY + Name, false))
                {
                    ...
                    if (original == null && Elevation.IsElevated)
                    {
                        SaveOriginalStartMode();
                        using (IRegistryKey reread = _registry.LocalMachine.OpenSubKey(SERVICES_REG_KEY + Name, false))
                            original = reread?.GetValue(SERVICE_ORIGINAL_START_VALUE_NAME, null);
                    }

                    if (original != null)
                        OriginalStartMode = (ServiceStartMode)original;
                    else
                        OriginalStartMode = _startMode;
                }
            }
            catch (Exception ex)
            {
                StaticViewModel.AddDebugMessage(ex, "Unable to determine original start mode for " + Name);
                OriginalStartMode = _startMode;
            }
        }
